...
using prior-season performance and draft capital.
...
Outputs data/oracle_predictions.json:
  ...
  backtest    — historical predicted vs actual wins (1996-2024)
...
def build_av_by_fy(picks):
    """Aggregate career AV and draft capital per (franchise, year), capped at AV_CAP_YEAR."""
    m = {}
    for p in picks:
...
def build_features(franchise, target_year, standings_map, capital_map, av_by_fy, global_av_rate):
    ...
    return [prev1['w'], p2w, pd_pg, 1 if prev1.get('playoff') else 0, cap, avr]


FEATURE_NAMES = ['prior_wins', 'prior2_wins', 'point_diff_pg', 'made_playoffs', 'draft_capital', 'recent_av_rate']
...
def main():
    ...
    for franchise in franchises:
        ...
        for year in range(TRAIN_FROM, TRAIN_TO + 1):
            if year not in sm:
                continue
            features = build_features(franchise, year, standings_map, capital_map, av_by_fy, global_av_rate)
            if features is None:
                continue
            X.append(features)
    ...
    forecast = []
    for franchise in franchises:
        features = build_features(franchise, 2026, standings_map, capital_map, av_by_fy, global_av_rate)
        if features is None:
            continue
    ...
    for row in forecast[:10]:
        ...
        print(f"  {row['franchise']:<32s} {row['predicted_wins']:>4.1f}W  (2025: {row['prior_wins']}W{marker})")
